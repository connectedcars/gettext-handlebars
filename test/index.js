...
describe('Parser', function () {
  ...
  describe('#parseToPoMessages()', function () {
    it('should return results', function (done) {
      fs.readFile(__dirname + '/fixtures/template.hbs', { encoding: 'utf8' }, function (err, data) {
        if (err) {
          throw err
        ...
        var result = (new Parser()).parseToPoMessages(data)
        ...
        var expected = [{
          ...
          "msgid": "This is a fixed sentence",
          "msgid_plural": null
        }, {
          "msgid": "This is wrapped in single quotes",
          "msgid_plural": null
        }, {
          ...
          "msgid": "inside block inverse",
          "msgid_plural": null
        }]

        for(var i=0; i<expected.length; i++) {
          assert.equal(result[i].msgid, expected[i].msgid)
          assert.equal(result[i].msgid_plural, expected[i].msgid_plural)
        }
        done()
      })
    })
    ...
    it('should return results with source path', function (done) {
      fs.readFile(__dirname + '/fixtures/plural.hbs', { encoding: 'utf8' }, function (err, data) {
        if (err) {
          throw err
        }

        var result = (new Parser()).parseToPoMessages(data, {sourcePath: '/my/file/path.hbs'})

        var expected = [{
          "msgid": "default",
          "msgid_plural": "defaults",
          "references": ["/my/file/path.hbs:1"]
        }, {
          "msgid": "quote",
          "msgid_plural": "quotes",
          "references": ["/my/file/path.hbs:2"]
        }]
        
        for(var i=0; i<expected.length; i++) {
          assert.equal(result[i].msgid, expected[i].msgid)
          assert.equal(result[i].msgid_plural, expected[i].msgid_plural)
          assert.equal(result[i].references[0], expected[i].references[0])
        }
    ...
    it('should parse files via glob patterns', function (done) {
      (new Parser()).parseFilesGlob(__dirname + '/fixtures/plural*.hbs', {}, function(er, result) {

        var expected = [{
          "msgid": "default",
          "msgid_plural": "defaults",
          "references": [
            __dirname + '/fixtures/plural-overlap.hbs:1',
            __dirname + '/fixtures/plural.hbs:1'
          ]
        }, {
          "msgid": "quote",
          "msgid_plural": "quotes",
          "references": [__dirname + '/fixtures/plural.hbs:2']
        }]
        ...
        for(var i=0; i<expected.length; i++) {
          ...
          for(var n=0; n<expected[i].references.length; n++) {
            assert.equal(result[i].references[n], expected[i].references[n])
